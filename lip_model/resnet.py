...
def resnet_18(input_tensor, repetitions=(2, 2, 2, 2)):
  ...
  #pool2 = tf.keras.layers.AveragePooling2D(pool_size=(block_shape[ROW_AXIS], block_shape[COL_AXIS]),strides=(1, 1))(block)

  #print('pool2', pool2.shape)

  flatten1 = tf.keras.layers.Flatten()(block)
...
def _shortcut(input, residual):
  ...
  if stride_width > 1 or stride_height > 1 or not equal_channels:
    shortcut = tf.keras.layers.Conv2D(filters=residual_shape[CHANNEL_AXIS], use_bias=False,
                      kernel_size=(1, 1),
...
def _conv_bn_relu(filters, kernel_size, strides=(1,1), padding='same', kernel_regularizer=kernel_regularizer):
  ...
  """
  def f(input):
    conv = tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, use_bias=False,
...
def _bn_relu_conv(filters, kernel_size, strides=(1,1), padding='same', kernel_regularizer=kernel_regularizer):
  ...
  def f(input):
    activation = _bn_relu(input)
    return tf.keras.layers.Conv2D( filters=filters, kernel_size=kernel_size, use_bias=False,
                  strides=strides, padding=padding,
                  kernel_initializer=kernel_initializer,
...
def _only_conv(filters, kernel_size, strides=(1,1), padding='same', kernel_regularizer=kernel_regularizer):
  ...
  def f(input):
    ...
    return tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, use_bias=False,
                  ...
                  kernel_initializer=kernel_initializer,
                  kernel_regularizer=kernel_regularizer )(padded)
